...
function romanToInt(rom) {
  ...
  let arrNumber = [1, 4, 5, 9, 10, 40, 50, 90, 100, 400, 500, 900, 1000]

  for (let i = 0; i < arrayStringRoman.length - 1; i++) {
    ...
    if (!arrString.includes(arrayStringRoman[i])) {
      ...
      const temp = arrayStringRoman[i]
      const max = arrayStringRoman(i + 1)
      if (temp > max) {
        max = temp
      }
      console.log(max)
